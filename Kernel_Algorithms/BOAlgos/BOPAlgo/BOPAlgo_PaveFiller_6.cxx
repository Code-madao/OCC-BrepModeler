...
void BOPAlgo_PaveFiller::MakeBlocks(const Message_ProgressRange& theRange)
{
  ...
  for (i=0; i<aNbFF; ++i, aPS.Next()) {
    ...
    // Map of all On/In V (PB + common V)
    aMVOnIn.Clear();
    // Map of all common On/In V (PB + V)
    aMVCommon.Clear();
    // Map of all On/In PBs
    aMPBOnIn.Clear();
    // Map of all common On/In PBs
    aMPBCommon.Clear();
    ...
    // Get the shared edges for nF1 and nF2 in aLSE
    ...
    aMVStick.Clear();
    aMVEF.Clear();
    // aMVStick is the new V belongs to nF1 and nF2
    // aMVEF is the new V in E/F and belongs to nF1 and nF2
                                          ...
                                          TColStd_MapOfInteger& aMI)
{
  ...
  //get original interference shape pair nS1,nS2
  //put the nVSD in aMVStick
   ...
   const Standard_Integer iCheckExtend)
{
  ...
  const Handle(BOPDS_PaveBlock)& aPB = aNC.PaveBlocks().First();
  const IntTools_Curve& aIC = aNC.Curve();
  //1. check if the nV is on the curve
  ...
  //
  if (bIsVertexOnLine) {
    // 1. 1 check if aPB contains the parameter aT
    ...
    //If contains aT = Pave , return the nVUesd = Pave.index
    // aDMVLV: <nVUsed, [nVUsed, nV]>
    // aMVTol: <nVUsed, aTolV>
